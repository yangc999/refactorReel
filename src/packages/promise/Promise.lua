...
function Promise.race(arr)
	...
	local co = coroutine.create(function()
		...
		while i < count do
			local promise, typ = promises[i], type(promises[i])
			if typ == 'table' and promise.andThen and promise[1] == PENDING then
...
function Promise.retry(func, maxRetry)
	return Promise.resolve(func()):catch(function(err)
		if maxRetry > 0 then
			return Promise.retry(func, maxRetry-1)
		else
